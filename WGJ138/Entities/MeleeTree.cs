...
namespace WGJ138.Entities {
    public class MeleeTree : Entity {
        ...
        public MeleeTree(Tile currentTile) : base(currentTile, new SpriteAnimation(0.45F, GameImpl.Textures[1, 1], GameImpl.Textures[1, 2])) {
            this.MoveRange = 1;
        }

        public override IEnumerable<IWait> MakeTurn(Gameplay gameplay) {
            yield return new WaitSeconds(1);
            var tiles = this.GetMovableTiles().ToArray();
